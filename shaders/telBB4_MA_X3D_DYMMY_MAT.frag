...
const float cons9 = float(1.000000000000);
uniform vec4 unf10;
uniform vec3 unf12;
const float cons20 = float(1.000000000000);
in vec3 varposition;
uniform float unf23;
uniform float unf24;
uniform float unf25;
...
void main()
{
	...
	set_rgb_zero(tmp2);
	set_rgb_zero(tmp3);
	shade_clamp_positive(vec4(tmp3, 1.0), tmp5);
	shade_clamp_positive(vec4(tmp2, 1.0), tmp7);
	shade_maddf(tmp7, cons9, unf10, tmp11);
	shade_add_mirror(unf12, tmp1, tmp11.rgb, tmp15);
	shade_add(vec4(tmp15, 1.0), tmp5, tmp18);
	mtex_alpha_to_col(tmp18, cons20, tmp21);
	shade_mist_factor(varposition, unf23, unf24, unf25, unf26, unf27, tmp28);
	mix_blend(tmp28, tmp21, unf31, tmp32);
	shade_alpha_opaque(tmp32, tmp34);
	linearrgb_to_srgb(tmp34, tmp36);
